...
while [ ${iteration} -le ${MAX_ITERATIONS} ] ; do
  ...
  array_Remove -a 'myArray' 0 1 4 5                       || fail 1
  ...
  array_Remove -a 'myAssoc' 'one' 'two' 'fourth element'     || fail 1
  ...
  array_Remove 'one' 'two' 2>/dev/null   && fail 1
  pass
  ...
  array_Remove -a 'myArray' 2>/dev/null   && fail 1
  pass
  ...
  array_Remove -a 'myArray' -p '[1-6]'                   || fail 1
  ...
  array_Remove -a 'myAssoc' -p '\b[a-z]{3}\b'                                                  || fail 1
  for token in "${!myAssoc[@]}" ; do
  ...
  array_Remove -a 'myArray' -n '2'                                                                                      || fail 1
  for token in "${!myArray[@]}" ; do
  ...
  array_Remove -a 'myAssoc' -n '2' || fail 1
  [ ${#myAssoc[@]} -eq 3 ]         || fail 2
  pass
  ...
  array_Remove -a 'myArray' -n '2' -p '[02]' '6'   || fail 1
  [ "${myArray[*]}" = 'four' ]                     || fail 2
  [ ${#myArray[@]} -eq 1 ]                         || fail 3
  [ "${!myArray[@]}" = '4' ]                       || fail 4
  pass
  ...
  array_Remove --array 'myArray' --nth '2' --pattern '[02]' '6'   || fail 1
  [ "${myArray[*]}" = 'four' ]                                    || fail 2
  [ ${#myArray[@]} -eq 1 ]                                        || fail 3
  [ "${!myArray[@]}" = '4' ]                                      || fail 4
  pass
