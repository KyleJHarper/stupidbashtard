...
while [ ${iteration} -le ${MAX_ITERATIONS} ] ; do
  ...
  [ "$(string_Pad -d 'right' -l 20 -p '-' 'some_string')" == 'some_string---------' ]  || fail 1
  [ "$(string_Pad -d 'left'  -l 20 -p '-' 'some_string')" == '---------some_string' ]  || fail 2
  [ "$(string_Pad -d 'both'  -l 20 -p '-' 'some_string')" == '----some_string-----' ]  || fail 3
  [ "$(string_Pad -d 'both'  -l 21 -p '-' 'some_string')" == '-----some_string-----' ] || fail 4
  pass

  # -- 2 -- Saving results in reference variable
  new_test "Storing results in reference variable: "
  rv=''
  string_Pad -d 'right' -l 20 -p '-' 'some_string' -R 'rv'
  ...
  [ "$(string_Pad -d 'right' -l 20 -p '-' -f '/tmp/string_Pad.tmp')" == 'some_string---------' ]  || fail 1
  pass
  rm /tmp/string_Pad.tmp

  # -- 4 -- Not specifying required items should fail
  new_test "Length is required, checking: "
  string_Pad -d 'right' -p '-' 'random junk' 2>/dev/null 1>/dev/null && fail 1
  pass

  # -- 5 -- Defaults shouldn't change
  new_test "Pad string and direction have defaults, ensuring they persist: "
  [ "$(string_Pad            -l 20 -p '-' 'some_string')" == 'some_string---------' ] || fail 1
  [ "$(string_Pad -d 'right' -l 20        'some_string')" == 'some_string         ' ] || fail 2
  [ "$(string_Pad            -l 20        'some_string')" == 'some_string         ' ] || fail 3
  pass
