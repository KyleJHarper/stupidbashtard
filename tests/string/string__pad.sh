...
while [ ${iteration} -le ${MAX_ITERATIONS} ] ; do
  ...
  [ "$(string__pad -d 'right' -l 20 -p '-' 'some_string')" == 'some_string---------' ]  || fail 1
  [ "$(string__pad -d 'left'  -l 20 -p '-' 'some_string')" == '---------some_string' ]  || fail 2
  [ "$(string__pad -d 'both'  -l 20 -p '-' 'some_string')" == '----some_string-----' ]  || fail 3
  [ "$(string__pad -d 'both'  -l 21 -p '-' 'some_string')" == '-----some_string-----' ] || fail 4
  pass

  # -- 2 -- Saving results in reference variable
  new_test "Storing results in reference variable: "
  rv=''
  string__pad -d 'right' -l 20 -p '-' 'some_string' -R 'rv'
  ...
  [ "$(string__pad -d 'right' -l 20 -p '-' -f '/tmp/string__pad.tmp')" == 'some_string---------' ]  || fail 1
  pass
  rm /tmp/string__pad.tmp

  # -- 4 -- Not specifying required items should fail
  new_test "Length is required, checking: "
  string__pad -d 'right' -p '-' 'random junk' 2>/dev/null 1>/dev/null && fail 1
  pass

  # -- 5 -- Defaults shouldn't change
  new_test "Pad string and direction have defaults, ensuring they persist: "
  [ "$(string__pad            -l 20 -p '-' 'some_string')" == 'some_string---------' ] || fail 1
  [ "$(string__pad -d 'right' -l 20        'some_string')" == 'some_string         ' ] || fail 2
  [ "$(string__pad            -l 20        'some_string')" == 'some_string         ' ] || fail 3
  pass
