...
while [ ${iteration} -le ${MAX_ITERATIONS} ] ; do
  ...
  [ "$(string__substring -i '4' -l '11' 'The_best_chance_is_now')" == 'best_chance' ] || fail 1
  pass

  # -- 2 -- Don't send a length
  new_test "Not specifying a length, should return the remainder after index: "
  [ "$(string__substring -i '4' 'The_best_chance_is_now')" == 'best_chance_is_now' ] || fail 1
  pass

  # -- 3 -- Long options
  new_test "Making sure long options work properly: "
  [ "$(string__substring --index '4' --length '11' 'The_best_chance_is_now')" == 'best_chance' ] || fail 1
  pass

  # -- 4 -- Negative index
  new_test "Sending a negative index, should start from the end of the string and NOT wrap forward: "
  [ "$(string__substring -i '-4' 'The_best_chance_is_now')" == '_now' ] || fail 1
  pass

  # -- 5 -- Negative length
  new_test "Sending a negative length should work, but has repercussions if boundaries are overflown: "
  [ "$(string__substring -l '-4' 'The_best_chance_is_now')" == 'The_best_chance_is' ]    || fail 1
  string__substring -l '-44' 'The_best_chance_is_now' 2>/dev/null 1>/dev/null            && fail 2
  [ "$(string__substring -i '4' -l '-4' 'The_best_chance_is_now')" == 'best_chance_is' ] || fail 3
  string__substring -i '20' -l '-4' 'The_best_chance_is_now' 2>/dev/null 1>/dev/null     && fail 4
  string__substring -i '-3' -l '-4' 'The_best_chance_is_now' 2>/dev/null 1>/dev/null     && fail 5
  [ "$(string__substring -i '-5' -l '-4' 'The_best_chance_is_now')" == 's' ]             || fail 6
  pass

  # -- 6 -- No options should send a warning, but work
  new_test "Sending no options should work, but give a warning: "
  __SBT_VERBOSE=true
  string__substring 'The_best_chance_is_now' 2>&1 | grep -q 'Both index and length are zero' || fail 1
  __SBT_VERBOSE=false
  pass

  # -- 7 -- Multiple strings should combine
  new_test "Multiple strings should be 'mashed' together: "
  [ "$(string__substring -i '4' 'The_best_chance_is_now' '_not_later')" == 'best_chance_is_now_not_later' ] || fail 1
  pass

  # -- 8 -- ByRef should be supported
  new_test "Storing value byref instead of stdout: "
  rawr=''
  string__substring -R 'rawr' -i '4' 'The_best_chance_is_now' '_not_later' || fail 1
