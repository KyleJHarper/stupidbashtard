...
while [ ${iteration} -le ${MAX_ITERATIONS} ] ; do
  ...
  # -- 7 -- Log file writing must be a real path.
  ...
  # -- 8 -- Log file writing can't use a relative path.
  ...
  # -- 9 -- Log file should write properly.
  new_test "Log file should write properly: "
  __SBT_LOG_FILE='/tmp/core__log_verbose__test_8.tmp'
  [ -f "${__SBT_LOG_FILE}" ] && rm "${__SBT_LOG_FILE}"
  core__log_verbose "Wheeeee data"  2>/dev/null                          || fail 1
  grep -qP '^\(main: [0-9]+\)  Wheeeee data$' "${__SBT_LOG_FILE}"        || fail 2
  [ -f "${__SBT_LOG_FILE}" ] && rm "${__SBT_LOG_FILE}"
  pass
