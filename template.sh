...
##
##@Description: This is an example template to help demonstrate how code should be written
##@Description: in the stupidbashtard modules.  It is important to follow the style guidelines
##@Description: so that the self-documenting system will work.  Also, if this is ever put
##@Description: into an IDE, it will enable intelligent description popups.
##@Description:
##@Description: This is how you skip a line.
##@Description:
##@Description: Using the same tag multiple times in succession is how you span lines.

##@Description: ^^ Empty lines mean nothing, but please avoid them for readability. A blank
##@Description: line (without the ##@Description tag) will not put a blank line in the
...
## caller, this is not advised.  As such, the document script will not look for non-local
## variables.
##
## -- Function Declarations
## You do NOT need to use special comments and tags.  You ONLY need to write the function using
## function keyword.  The documentation tool will read everything after it, including getopts.
##
