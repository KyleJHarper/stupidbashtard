...
function string_IndexOf {
  ...
  for _temp in "${__SBT_NONOPT_ARGS[@]}" ; do _data+="${_temp}" ; done
  core_ReadDATA "${_files[@]}" || return 1
  core_ToolExists 'awk' -v '-W version' || return 1
  ...
  for _needle in "${_needles[@]}" ; do
    ...
    let "_index += $(awk -v haystack="${_data}" -v needle="${_needle}" -v occurrence="${_occurrence}" -f "${__SBT_EXT_DIR}/string_IndexOf.awk")"
    [ ${_index} -eq -1 ] || break
  done
...
function string_Reverse {
  ...
  _temp="$(awk -f "${__SBT_EXT_DIR}/string_Reverse.awk" <<<"${_data}")"
