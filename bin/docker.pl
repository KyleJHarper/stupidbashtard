...
foreach ( @FILES ) {
  ...
  while ( <$fh> ) {
    ...
    if ( /^[\s]*#(?!@)/ ) { next ; }  # Comments mean nothing, unless it's the docker tag format:  #@
    ...
    if ( ! $func->name() ) {
      if ( /^[\s]*#@([\S]+)[\s]+(.*)?$/ ) {
        # We're not in a function, but we have a docker tag:  #@something.  Store it as a namespace tag.  Replace '-' with a newline, per standards.
